...
class EEPfile:
    ...
    def plot_vars(self, xvar, yvar, xlabel='', ylabel='', color='black', linewidth=1.5, linestyle='-', label='', loc=1, pltnum = 1, x_inv=0, y_inv=0, saveplot=0, figname='', phase=None, phasecolor=''):
        ...
        Keywords:
            ...
            mesa_plot_hist keyword: pltnum, x_inv, y_inv, saveplot, figname, phase, phasecolor
        ...
        if phase != None:
            ...
            if len(p_ind) > 0:
                if phasecolor == '':
                    ...
                    ax.plot(x[p_ind], y[p_ind], color=phasecolor, linewidth=4.0, alpha=0.3)
        
        return x, y
                
    def plot_HR(self, color='black', linestyle='-', linewidth=1.5, label='', loc=1, pltnum=2, logg=False, figname='', saveplot=0, phase=None, phasecolor=''):
        ...
        Keywords:
            ...
            mesa_plot_hist keyword: pltnum, logg, figname, saveplot, phase, phasecolor
        ...
        if logg == False:
            logTeff, logL = self.plot_vars('log_Teff','log_L', pltnum=pltnum, color=color, linestyle=linestyle, linewidth=linewidth, label=label, loc=3, xlabel=r'$\log(T_{\rm eff})\;[\rm K]$', ylabel=r'$\log(L/L_{\odot}$)', x_inv=1, saveplot=saveplot, figname=figname, phase=phase, phasecolor=phasecolor)
            return logTeff, logL
            
        #plot the regular HR diagram
        else:
            logTeff, logg = self.plot_vars('log_Teff','log_g', pltnum=pltnum, color=color, linestyle=linestyle, linewidth=linewidth, label=label, loc=3, xlabel=r'$\log(T_{\rm eff})\;[\rm K]$', ylabel=r'$\log(g)\;[\rm g\;cm^{-3}]$', x_inv=1, y_inv=1, saveplot=saveplot, figname=figname, phase=phase, phasecolor=phasecolor)
            return logTeff, logg
