...
class CMD:
    ...
    def __init__(self, filename, verbose=True):
        ...
        Usage:
            >> cmd = read_mist_models.CMD('MIST_v1.0_feh_p0.00_afe_p0.0_vvcrit0.4.iso.cmd')
            >> age_ind = cmd.age_index(7.0)
            >> B = cmd.cmds[age_ind]['Bessell_B']
            >> V = cmd.cmds[age_ind]['Bessell_V']
        ...
        Attributes:
            ...
            extinction  Av for CCM89 extinction.
        ...
        self.version, self.photo_sys, self.abun, self.Av_extinction, self.rot, self.ages, self.num_ages, self.hdr_list, self.cmds = self.read_cmd_file()
    ...
    def read_cmd_file(self):
        ...
        for i_age in range(num_ages):
            ...
            cmd = np.zeros((num_eeps),{'names':tuple(hdr_list),'formats':tuple(formats)})
            #read through EEPs for each isochrone
            for eep in range(num_eeps):
                cmd_chunk = data[3+counter+eep]
                cmd[eep]=tuple(cmd_chunk)
            cmd_set.append(cmd)
            ages.append(cmd[0][1])
            counter+= 3+num_eeps+2
        return version, photo_sys, abun, Av_extinction, rot, ages, num_ages, hdr_list, cmd_set
...
class EEP:
    ...
    def plot_HR(self, fignum=0, phases=[None], phasecolor=[''], **kwargs):
        ...
        plt.xlabel(r'$\log(T_{\rm eff})\;[\rm K]$', fontsize=22)
        plt.ylabel(r'$\log(L/L_{\odot}$)', fontsize=22)
